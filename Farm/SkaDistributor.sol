...
contract SkaDistributor is Ownable, ReentrancyGuard {
    ...
    // Update the given pool's Ska allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // Deposit LP tokens to MasterChef for Ska allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(
            block.number >= startBlock,
            "SkaDistributor: Can not deposit before start"
        );
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(ska)) {
            totalSkaInPools = totalSkaInPools.sub(amount);
        }
    ...
    function safeSkaTransfer(address _to, uint256 _amount) internal {
        if (ska.balanceOf(address(this)) > totalSkaInPools) {
            //SkaBal = total Ska in SkaDistributor - total Ska in Ska pools, this will make sure that SkaDistributor never transfer rewards from deposited Ska pools
